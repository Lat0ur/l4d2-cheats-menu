...
vec2_t c_menu::mouse_pos() {
	...
	ScreenToClient(FindWindowA(0, "Left 4 Dead 2"), &mouse_position);
...
void c_menu::draw() {
	...
	if (!_pressed && GetAsyncKeyState(VK_INSERT))
		...
		interfaces::surface_interface->SetCursorAlwaysVisible(menu_opened);
	}

	interfaces::engine_interface->get_screen_size(screen_width, screen_width);
	...
	if (menu_opened) {
		...
		RECT netflix_text_size = get_text_size(render_manager::fonts::cool_thing_font, "Undefeated");

		render_manager::text("Undefeated", _pos.x + _width - 5 - netflix_text_size.right, _pos.y + _height - 36, render_manager::fonts::cool_thing_font, color(127, 193, 255, 235));
