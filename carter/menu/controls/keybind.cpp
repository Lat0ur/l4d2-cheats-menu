...
void c_menu::keybind(int &item, std::string name) {
	...
	if (open) {
		if (listen_for_input(item)) {
			something = interfaces::Inputsystem->button_code_to_string((button_code_t)item);
		}
		else {
			something = "press a key";
		}
	}
	else something = interfaces::Inputsystem->button_code_to_string((button_code_t)item);
